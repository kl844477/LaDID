...
class AttentionBase(IAttention):
    ...
    def forward(self, x: Tensor, return_weights: bool = True) -> Union[tuple[Tensor, Tensor], tuple[Tensor, None]]:
        ...
        if self.__class__.__name__ in ['SpatioTemporalDotProductAttention']:
                attn_weights = self._eval_attn_weights(rpe)
        elif self.__class__.__name__ in ['SpatioTemporalDotProductAttention_V2']:
                attn_weights = self._eval_attn_weights(x)
                tmp_attn_weights = self._eval_tmp_attn_weights(rpe)
                attn_weights = attn_weights + tmp_attn_weights
        else:
            attn_weights = self._eval_attn_weights(x)
    ...
    def _eval_tmp_attn_weights(self, x: Tensor) -> Tensor:
        raise NotImplementedError()
...
class SpatioTemporalDotProductAttention(AttentionBase):
    def __init__(self, d_model: int,  rpe: Union[Module, None] = None, **kwargs):
        super().__init__(d_model, rpe,)
        self.W_k = nn.Linear(self.d_model, self.d_model, bias=False)
        self.W_v = nn.Linear(self.d_model, self.d_model, bias=False)
...
class SpatioTemporalDotProductAttention_V2(AttentionBase):
    def __init__(self, d_model: int, rpe: Union[Module, None] = None, **kwargs):
        super().__init__(d_model, rpe,)
        self.W_k = nn.Linear(self.d_model, self.d_model, bias=False)
        self.W_v = nn.Linear(self.d_model, self.d_model, bias=False)
        ...
        self.W_k_tmp = nn.Linear(self.d_model, self.d_model, bias=False)
        self.W_q_tmp = nn.Linear(self.d_model, self.d_model, bias=False)
    ...
    def _eval_tmp_attn_weights(self, x: Tensor) -> Tensor:
        Q, K = self.W_q_tmp(x), self.W_k_tmp(x)
        unnorm_attn_weights = torch.bmm(Q, torch.transpose(K, 1, 2)) / self.d_model**0.5
        attn_weights = nn.Softmax(-1)(unnorm_attn_weights)
        return attn_weights
